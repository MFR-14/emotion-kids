...
window.addEventListener("DOMContentLoaded", ()=>{
  ...
  const level  = (p.get("level")  || localStorage.getItem("ek_last_level") || "3").trim();

  // skor bisa beda tiap level, jadi fallback-nya kita cek berdasarkan level terakhir
  const skorUrl = (p.get("skor") || "").trim();
  const skorLs  = (level === "3")
    ? (localStorage.getItem("ek_level3_skor") || "0")
    : (localStorage.getItem(`ek_level${level}_skor`) || "0");

  const skor   = (skorUrl || skorLs).trim();
  const alasan = (p.get("alasan") || localStorage.getItem("ek_level3_alasan") || "Selesai").trim();
